    ...
    methods: {
        ...
        showLink: async function(data){
            ...
            if (this.internal_external === "Internal"){
                ...
                if (this.assignedTicketData.type === "Starbooks related"){
                    console.log(this.assignedTicketData.type);
                    ...
                    let a8 = currentDateTime; // date requested
                    let a17 = this.assignedTicketData.sex.replaceAll(" ", "+"); //sex
                    let a3 = "Technical Support Services".replaceAll(" ", "+"); //type of services
                    ...
                    let a12 = "Technical Support".replaceAll(" ", "+");
                    let a13 = this.assignedTicketData.externalOtherType_.replaceAll(" ", "+");
                    let a14 = this.assignedTicketData.attended_by.replaceAll(" ", "+");
                    let a15 = this.assignedTicketData.remarks.replaceAll(" ", "+");
                    let a16 = this.assignedTicketData.assisted_by_1.replaceAll(" ", "+");

                    let link_sb = "https://docs.google.com/forms/d/e/1FAIpQLSe0V3B6Hoh7taNfYhp-agkbmTTQbk2Fcp2GkL7ne34u5lQpng/viewform?usp=pp_url"
                    + "&entry.204278290=" + a8
                    + "&entry.468511028=" + a17
                    + "&entry.1416427164=" + a8
                    + "&entry.2016337212=" + a17
                    + "&entry.2008685738=" + a3
                    + "&entry.1725600=" + "0"
                    + "&entry.1819805617=" + "0"
                    + "&entry.1637951943=" + a5
                    ...
                    + "&entry.1650701328=" + a8
                    + "&entry.910998812=" + a10
                    + "&entry.1859630267=" + a11
                    + "&entry.1734376917=" + a12
                    + "&entry.1517158863=" + "NA" 
                    + "&entry.9829575=" + a14
                    + "&entry.411227260=" + a16;
                    
                    const textarea = document.createElement('textarea');
                    textarea.value = link_sb;
                    //hide textarea from UI (trick 🙂 )
                    textarea.style.position = 'fixed';
                    textarea.style.left = '-9999px';
                    document.body.appendChild(textarea);
                ...
                else if (this.assignedTicketData.type != "Starbooks related"){
                    const date = new Date(this.assignedTicketData.date1);
                    let currentDay= String(date.getDate()).padStart(2, '0');
                    let currentMonth = String(date.getMonth()+1).padStart(2,"0");
                    let currentYear = date.getFullYear();
